...
static void level_cmd_begin_area(void) {
    ...
    if (areaIndex < 8) {
        ...
        struct GraphNodeRoot *screenArea2 =
            (struct GraphNodeRoot *) process_geo_layout(sLevelPool, geoLayoutAddr);
        struct GraphNodeCamera *node2 = (struct GraphNodeCamera *) screenArea2->views[0];
        ...
        if (node != NULL) {
            ...
            gAreas[areaIndex].luigiCamera = NULL;
            gAreas[areaIndex].marioCamera = NULL;
        }
    }
