...
static void handle_cursor_button_input(void) {
    ...
    if (sSelectedButtonID == MENU_BUTTON_SCORE_FILE_A || sSelectedButtonID == MENU_BUTTON_SCORE_FILE_B
        ...
        if ((gPlayer3Controller->buttonPressed | gPlayer2Controller->buttonPressed) & (B_BUTTON | START_BUTTON)) {
            sClickPos[0] = sCursorPos[0];
            sClickPos[1] = sCursorPos[1];
            sCursorClickingTimer = 1;
        } else if ((gPlayer3Controller->buttonPressed | gPlayer2Controller->buttonPressed) & A_BUTTON) {
            sScoreFileCoinScoreMode = 1 - sScoreFileCoinScoreMode;
            play_sound(SOUND_MENU_CLICK_FILE_SELECT, gDefaultSoundArgs);
        }
    } else { // If cursor is clicked
        if ((gPlayer3Controller->buttonPressed | gPlayer2Controller->buttonPressed) & (A_BUTTON | B_BUTTON | START_BUTTON)) {
            sClickPos[0] = sCursorPos[0];
            sClickPos[1] = sCursorPos[1];
            sCursorClickingTimer = 1;
        }
...
static void handle_controller_cursor_input(void) {
    s16 rawStickX = gPlayer3Controller->rawStickX + gPlayer2Controller->rawStickX;
    s16 rawStickY = gPlayer3Controller->rawStickY + gPlayer2Controller->rawStickY;
