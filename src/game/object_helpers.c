...
s32 geo_switch_area(s32 run, struct GraphNode *node) {
    ...
    if (run == TRUE) {
        if (gMarioObject == NULL) {
            ...
            find_floor(gMarioStates[gCurrLevelCamera->cameraID].marioObj->oPosX,
                       gMarioStates[gCurrLevelCamera->cameraID].marioObj->oPosY,
...
void moveOBJ(struct Object *t) {
    t->oVelX = t->oForwardVel * sins(t->oMoveAngleYaw);
    t->oVelZ = t->oForwardVel * coss(t->oMoveAngleYaw);
...
void func_802A11B4(UNUSED struct Object *a0, s32 a1) {
    gPlayerStatusForCamera[gCurrLevelCamera->cameraID].unk1C[1] = (s16) a1;
    gSecondCameraFocus[gCurrLevelCamera->cameraID] = o;
}
...
void obj_push_mario_away(f32 radius, struct MarioState *m) {
    f32 marioRelX = m->marioObj->oPosX - o->oPosX;
    f32 marioRelZ = m->marioObj->oPosZ - o->oPosZ;
...
void obj_push_mario_away_from_cylinder(f32 radius, f32 extentY) {
    ...
    for (i = 0; i < activePlayers; i++) {
        ...
        if (marioRelY < extentY) {
            obj_push_mario_away(radius, &gMarioStates[i]);
        }
    }
}
...
s32 func_802A3FF8(f32 radius, f32 height, UNUSED s32 unused) {
    ...
    if (o->oDistanceToMario < 1500.0f) {
        ...
        if (latDistToMario < radius && o->oPosY < gMarioObject->oPosY + 160.0f
            && gMarioObject->oPosY < o->oPosY + height && !(m->action & ACT_FLAG_AIR)
