...
static void set_swimming_at_surface_particles(struct MarioState *m, u32 particleFlag) {
    ...
    if (atSurface) {
        ...
        if (atSurface ^ sWasAtSurface) {
            play_sound(SOUND_ACTION_UNKNOWN431, m->marioObj->soundOrigin);
        }
    }
...
static void func_802713A8(struct MarioState *m) {
    ...
    if (animFrame == 0 || animFrame == 12) {
        play_sound(SOUND_ACTION_UNKNOWN434, m->marioObj->soundOrigin);
    }
}
...
static s32 act_hold_breaststroke(struct MarioState *m) {
    ...
    if (m->actionTimer == 1) {
        play_sound(SOUND_ACTION_UNKNOWN433, m->marioObj->soundOrigin);
        func_8027107C(m);
    }
...
static s32 act_water_plunge(struct MarioState *m) {
    ...
    if (m->actionState == 0) {
        ...
        if (m->peakHeight - m->pos[1] > 1150.0f) {
            play_sound(SOUND_MARIO_HAHA_2, m->marioObj->soundOrigin);
        }
...
static void play_metal_water_walking_sound(struct MarioState *m) {
    if (is_anim_past_frame(m, 10) || is_anim_past_frame(m, 49)) {
        play_sound(SOUND_ACTION_UNKNOWN452, m->marioObj->soundOrigin);
