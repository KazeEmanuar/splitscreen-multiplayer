...
s32 mario_update_punch_sequence(struct MarioState *m) {
    ...
    switch (m->actionArg) {
        case 0:
            play_sound(SOUND_MARIO_PUNCH_YAH, m->marioObj->soundOrigin);
            // Fall-through:
        case 1:
        ...
        case 3:
            play_sound(SOUND_MARIO_PUNCH_WAH, m->marioObj->soundOrigin);
            // Fall-through:
        case 4:
...
s32 act_picking_up_bowser(struct MarioState *m) {
    if (m->actionState == 0) {
        ...
        mario_grab_used_object(m);
        play_sound(SOUND_MARIO_HRMM, m->marioObj->soundOrigin);
    }
...
s32 act_holding_bowser(struct MarioState *m) {
    ...
    if (m->input & INPUT_B_PRESSED) {
        ...
        if (m->angleVel[1] <= -0xE00 || m->angleVel[1] >= 0xE00) {
            play_sound(SOUND_MARIO_SO_LONGA_BOWSER, m->marioObj->soundOrigin);
        } else {
    ...
    if (m->angleVel[1] <= -0x100 && spin < m->faceAngle[1]) {
        play_sound(SOUND_OBJ_BOWSER_SPINNING, m->marioObj->soundOrigin);
    }
    if (m->angleVel[1] >= 0x100 && spin > m->faceAngle[1]) {
        play_sound(SOUND_OBJ_BOWSER_SPINNING, m->marioObj->soundOrigin);
    }
