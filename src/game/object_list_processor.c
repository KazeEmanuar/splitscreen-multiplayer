...
const u16 hitboxAnimCount = sizeof(multiplayerHurtboxAnims);

u8 multiplayerHurtboxAnimsMoveless[] = {
    /* 0x3D */ MARIO_ANIM_GROUND_POUND,
    /* 0x47 */ MARIO_ANIM_RIDING_SHELL,
    /* 0x4A */ MARIO_ANIM_JUMP_RIDING_SHELL,
    /* 0x54 */ MARIO_ANIM_BEND_KNESS_RIDING_SHELL, // unused
...
const u16 hitboxMovelessAnimCount = sizeof(multiplayerHurtboxAnimsMoveless);
 ...
 * object.
 */
void copy_mario_state_to_object(int i) {
  ...
  //      if (gMarioState->controller->buttonPressed & L_TRIG){
   //         level_trigger_warp(gMarioState, WARP_OP_CREDITS_START);
    ...
    if (activePlayers > 1) {
        ...
        copy_object_pos_and_angle(playerHitbox, gCurrentObject);
        copy_object_pos_and_angle(playerHurtbox, gCurrentObject);
        playerHitbox->oInteractType = INTERACT_IGLOO_BARRIER;
        playerHurtbox->oInteractType = INTERACT_BOUNCE_TOP;

        x = gMarioStates[l].pos[0] - gMarioStates[l ^ 1].pos[0];
        ...
        playerHurtbox->oIntangibleTimer = -1;
        if (hitMario) {
            playerHurtbox->oDamageOrCoinValue = 2;
            ...
            playerHurtbox->oForwardVel = 0.0f;
            if (iterate == hitboxMovelessAnimCount) {
                playerHurtbox->oForwardVel = 30.0f;
            }
            moveOBJ(playerHurtbox);
        }
    }
}
