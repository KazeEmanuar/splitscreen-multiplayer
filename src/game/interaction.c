...
u32 interact_unknown_31(struct MarioState *, u32, struct Object *);
...
static struct InteractionHandler sInteractionHandlers[] = {
    ...
    { INTERACT_UNKNOWN_31, interact_bounce_top },
};
...
void bounce_off_object(struct MarioState *m, struct Object *o, f32 velY) {
    ...
    play_sound(SOUND_ACTION_UNKNOWN459, m->marioObj->soundOrigin);
}
...
void bounce_back_from_attack(struct MarioState *m, u32 interaction) {
    ...
    if (interaction & (INT_PUNCH | INT_KICK | INT_TRIP | INT_FAST_ATTACK_OR_SHELL)) {
        play_sound(SOUND_ACTION_UNKNOWN444_2, m->marioObj->soundOrigin);
    }
}
...
u32 take_damage_and_knock_back(struct MarioState *m, struct Object *o) {
    u32 damage;
    if (!m->sInvulnerable && !(m->flags & MARIO_VANISH_CAP)) {
...
u32 interact_star_or_key(struct MarioState *m, UNUSED u32 interactType, struct Object *o) {
    ...
    if (m->health >= 0x100) {
        ...
        play_sound(SOUND_MENU_STAR_SOUND, m->marioObj->soundOrigin);
#ifndef VERSION_JP
        update_mario_sound_and_camera(m);
        // func_802521A0
#endif
...
u32 interact_warp(struct MarioState *m, UNUSED u32 interactType, struct Object *o) {
    ...
    if (o->oInteractionSubtype & INT_SUBTYPE_FADING_WARP) {
        ...
        } else if (!sJustTeleported && (timeSinceWarp > 200)) {
...
u32 interact_flame(struct MarioState *m, UNUSED u32 interactType, struct Object *o) {
    ...
    if (!m->sInvulnerable && !(m->flags & MARIO_METAL_CAP) && !(m->flags & MARIO_VANISH_CAP)
        ...
        if ((m->action & (ACT_FLAG_SWIMMING | ACT_FLAG_METAL_WATER))
            ...
            play_sound(SOUND_GENERAL_FLAME_OUT, m->marioObj->soundOrigin);
        } else {
            m->marioObj->oMarioBurnTimer = 0;
            update_mario_sound_and_camera(m);
            play_sound(SOUND_MARIO_ON_FIRE, m->marioObj->soundOrigin);
...
u32 interact_snufit_bullet(struct MarioState *m, UNUSED u32 interactType, struct Object *o) {
    if (!m->sInvulnerable && !(m->flags & MARIO_VANISH_CAP)) {
        if (m->flags & MARIO_METAL_CAP) {
            ...
            play_sound(SOUND_ACTION_UNKNOWN458, m->marioObj->soundOrigin);
        } else {
            o->oInteractStatus = INT_STATUS_INTERACTED | INT_STATUS_ATTACKED_MARIO;
            m->interactObj = o;
...
u32 interact_bully(struct MarioState *m, UNUSED u32 interactType, struct Object *o) {
    ...
    else if (!m->sInvulnerable && !(m->flags & MARIO_VANISH_CAP)
             && !(o->oInteractionSubtype & INT_SUBTYPE_DELAY_INVINCIBILITY)) {
        o->oInteractStatus = INT_STATUS_INTERACTED;
        m->invincTimer = 2;

        update_mario_sound_and_camera(m);
        play_sound(SOUND_MARIO_EEUH, m->marioObj->soundOrigin);
        play_sound(SOUND_OBJ_BULLY_METAL, m->marioObj->soundOrigin);
    ...
    if (interaction & INT_ANY_ATTACK) {
        ...
        if (interaction & INT_HIT_FROM_ABOVE) {
            if (o->oInteractionSubtype & INT_SUBTYPE_TWIRL_BOUNCE) {
                ...
                reset_mario_pitch(m);
#ifndef VERSION_JP
                play_sound(SOUND_MARIO_TWIRL_BOUNCE, m->marioObj->soundOrigin);
#endif
...
void mario_process_interactions(struct MarioState *m) {
    struct Object *playerHurtbox = m->marioObj->unused2;
    ...
    if (!(m->action & ACT_FLAG_INTANGIBLE) && m->collidedObjInteractTypes != 0) {
        ...
        for (i = 0; i < 31; i++) {
            ...
            if (m->collidedObjInteractTypes & interactType) {
                ...
                if ((m->marioObj->unused1 != object) && (playerHurtbox != object)) {
    ...
    if (m->invincTimer > 0 && !m->sDelayInvincTimer) {
        m->invincTimer -= 1;
    }
