...
void bhv_camera_lakitu_update(void) {
    if (!(o->activeFlags & ACTIVE_FLAG_IN_DIFFERENT_ROOM)) {
        ...
        if (o->oBehParams2ndByte != CAMERA_LAKITU_BP_FOLLOW_CAMERA) {
            ...
            f32 val0C = (f32) 0x875C3D / 0x800 - gCameraStatus[0].camFocAndPosCurrAndGoal[0][3];
            if (gCameraStatus[0].camFocAndPosCurrAndGoal[0][3] < 1700.0f || val0C < 0.0f) {
                obj_hide();
            } else {
                obj_unhide();

                o->oPosX = gCameraStatus[0].camFocAndPosCurrAndGoal[0][3];
                o->oPosY = gCameraStatus[0].camFocAndPosCurrAndGoal[0][4];
                o->oPosZ = gCameraStatus[0].camFocAndPosCurrAndGoal[0][5];

                o->oHomeX = gCameraStatus[0].camFocAndPosCurrAndGoal[0][0];
                o->oHomeZ = gCameraStatus[0].camFocAndPosCurrAndGoal[0][2];

                o->oFaceAngleYaw = -obj_angle_to_home();
                o->oFaceAnglePitch = atan2s(obj_lateral_dist_to_home(),
                                            o->oPosY - gCameraStatus[0].camFocAndPosCurrAndGoal[0][1]);
