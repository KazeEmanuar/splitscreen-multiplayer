...
void cutscene_take_cap_off(struct MarioState *m) {
    ...
    play_sound(SOUND_ACTION_UNKNOWN43D, m->marioObj->soundOrigin);
}
...
void cutscene_put_cap_on(struct MarioState *m) {
    m->flags &= ~MARIO_CAP_IN_HAND;
    m->flags |= MARIO_CAP_ON_HEAD;
    play_sound(SOUND_ACTION_UNKNOWN43E, m->marioObj->soundOrigin);
}
...
s32 act_reading_sign(struct MarioState *m) {
    ...
    switch (m->actionState) {
        ...
        case 2:
            // dialog finished
            if (m->thisPlayerCamera->cutscene == 0) {
                disable_time_stop();
                set_mario_action(m, ACT_IDLE, 0);
            }
            break;
...
void general_star_dance_handler(struct MarioState *m, s32 isInWater) {
    ...
    if (m->actionState == 0) {
        switch (++m->actionTimer) {
            ...
            case 42:
                play_sound(SOUND_MARIO_HERE_WE_GO, m->marioObj->soundOrigin);
                break;
...
s32 act_star_dance(struct MarioState *m) {
    m->faceAngle[1] = m->thisPlayerCamera->trueYaw;
...
s32 act_fall_after_star_grab(struct MarioState *m) {
    if (m->pos[1] < m->waterLevel - 130) {
        play_sound(SOUND_ACTION_UNKNOWN430, m->marioObj->soundOrigin);
...
s32 act_quicksand_death(struct MarioState *m) {
    ...
    play_sound(SOUND_MOVING_QUICKSAND_DEATH, m->marioObj->soundOrigin);
    return FALSE;
}
...
s32 act_unlocking_key_door(struct MarioState *m) {
    ...
    switch (m->marioObj->header.gfx.unk38.animFrame) {
        case 79:
            play_sound(SOUND_GENERAL_SWITCH4, m->marioObj->soundOrigin);
            break;
        case 111:
            play_sound(SOUND_GENERAL_SWITCH2, m->marioObj->soundOrigin);
            break;
    }
...
s32 act_spawn_spin_airborne(struct MarioState *m) {
    ...
    if (m->actionState == 0 && m->pos[1] - m->floorHeight > 300.0f) {
        if (set_mario_animation(m, MARIO_ANIM_FORWARD_SPINNING) == 0) { // first anim frame
            play_sound(SOUND_ACTION_SWISH2, m->marioObj->soundOrigin);
        }
    }
    ...
    switch (m->actionState) {
        ...
        case 1:
            ...
            switch (animFrame) {
                case -1:
                    ...
                    //! fall through
                case 67:
                    play_sound(SOUND_ACTION_SWISH1, m->marioObj->soundOrigin);
                    //! fall through
                case 83:
                    play_sound(SOUND_ACTION_PAT_BACK, m->marioObj->soundOrigin);
                    //! fall through
                case 111:
                    play_sound(SOUND_ACTION_UNKNOWN45C, m->marioObj->soundOrigin);
                    // no break
            }
            handle_save_menu(m);
            break;
        ...
        case 3:
            ...
            switch (animFrame) {
                ...
                // fall through
                case 53:
                    play_sound(SOUND_ACTION_BRUSH_HAIR, m->marioObj->soundOrigin);
                    break;
                case 82:
...
#ifdef VERSION_JP
        play_sound(SOUND_MARIO_OOOF, m->marioObj->soundOrigin);
#else
        play_sound(SOUND_MARIO_OOOF2, m->marioObj->soundOrigin);
#endif
        m->numLives;
    ...
    if (m->actionTimer++ == 32) {
        if (m->pos[1] < m->waterLevel - 100) {
            ...
            if (m->thisPlayerCamera->currPreset != CAMERA_PRESET_WATER_SURFACE) {
                ...
                func_80285BD8(m->thisPlayerCamera, 8, 1);
            }
...
static void intro_cutscene_raise_pipe(struct MarioState *m) {
    ...
    if (m->actionTimer == 0) {
        play_sound(SOUND_MENU_EXIT_PIPE, sIntroWarpPipeObj->soundOrigin);
    }
...
static void intro_cutscene_jump_out_of_pipe(struct MarioState *m) {
    ...
    if (m->actionTimer++ >= 118) {
        ...
        if (perform_air_step(m, 0) == AIR_STEP_LANDED) {
            ...
            play_mario_landing_sound(m, SOUND_ACTION_TERRAIN_LANDING);
#ifndef VERSION_JP
            play_sound(SOUND_MARIO_HAHA, m->marioObj->soundOrigin);
#endif
            advance_cutscene_step(m);
        }
    }
...
static void intro_cutscene_lower_pipe(struct MarioState *m) {
    if (m->actionTimer++ == 0) {
        play_sound(SOUND_MENU_ENTER_PIPE, sIntroWarpPipeObj->soundOrigin);
        set_mario_animation(m, MARIO_ANIM_FIRST_PERSON);
    }
...
static s32 jumbo_star_cutscene_taking_off(struct MarioState *m) {
    ...
    if (m->actionState == 0) {
        ...
        switch (animFrame) {
            ...
            case 28:
                play_sound(SOUND_MARIO_HOOHOO, m->marioObj->soundOrigin);
                break;

            case 60:
                play_sound(SOUND_MARIO_YAHOO, m->marioObj->soundOrigin);
                break;
        }
        m->particleFlags |= PARTICLE_SPARKLES;
...
static void end_peach_cutscene_spawn_peach(struct MarioState *m) {
    ...
    if (m->actionTimer >= 40) {
        play_sound(SOUND_AIR_PEACH_TWINKLE, sEndPeachObj->soundOrigin);
    }
}
...
static void end_peach_cutscene_dialog_1(struct MarioState *m) {
    ...
    switch (m->actionTimer) {
        ...
        case 230:
            ...
            play_sound(SOUND_PEACH_MARIO, sEndPeachObj->soundOrigin);
#endif
            break;
...
#ifndef VERSION_JP
            play_sound(SOUND_PEACH_POWER_OF_THE_STARS, sEndPeachObj->soundOrigin);
#endif
            break;
...
#ifndef VERSION_JP
            play_sound(SOUND_PEACH_THANKS_TO_YOU, sEndPeachObj->soundOrigin);
#endif
            break;
...
#ifndef VERSION_JP
            play_sound(SOUND_PEACH_SOMETHING_SPECIAL, sEndPeachObj->soundOrigin);
#endif
            break;
...
#ifndef VERSION_JP
            play_sound(SOUND_PEACH_FOR_MARIO, sEndPeachObj->soundOrigin);
#endif
            break;
    }
...
static s32 act_credits_cutscene(struct MarioState *m) {
    ...
    if (m->pos[1] < m->waterLevel - 100) {
        if (m->thisPlayerCamera->currPreset != CAMERA_PRESET_BEHIND_MARIO) {
            func_80285BD8(m->thisPlayerCamera, 3, 1);
        }
