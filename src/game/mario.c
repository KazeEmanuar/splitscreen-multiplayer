...
void play_sound_and_spawn_particles(struct MarioState *m, u32 soundBits, u32 waveParticleType) {
    ...
    if ((m->flags & MARIO_METAL_CAP) || soundBits == SOUND_ACTION_UNKNOWN443
        ...
        play_sound(m->terrainSoundAddend + soundBits, m->marioObj->soundOrigin);
    }
}
 ...
 */
s32 execute_mario_action(UNUSED struct Object *o, int i) {
    s32 inLoop = TRUE;
...
void init_mario(void) {
    ...
    for (i = 0; i < activePlayers; i++) {
        ...
        if (i == 0) {
            ...
            gMarioStates[i].marioObj = gLuigiObject;
        }
        gMarioStates[i].marioObj->header.gfx.unk38.animID = -1;
        ...
        if (i == 0) {
            gMarioStates[i].pos[0] += 35.0f * sins(gMarioStates[i].faceAngle[1] - 0x4000);
            gMarioStates[i].pos[2] += 35.0f * coss(gMarioStates[i].faceAngle[1] - 0x4000);
        } else {
            gMarioStates[i].pos[0] += 35.0f * sins(gMarioStates[i].faceAngle[1] + 0x4000);
            gMarioStates[i].pos[2] += 35.0f * coss(gMarioStates[i].faceAngle[1] + 0x4000);
        }
        ...
        gMarioStates[i].floorHeight = find_floor(gMarioStates[i].pos[0], gMarioStates[i].pos[1],
                                                 gMarioStates[i].pos[2], &gMarioStates[i].floor);
...
void init_mario_from_save_file(void) {
    ...
    for (i = 0; i < activePlayers; i++) {
        ...
        gMarioStates[i].unkB8 = gMarioStates[i].numStars;
        gMarioStates[i].unkB0 = 0xBD;

        gHudDisplay.coins = 0;
        gHudDisplay.wedges[i] = 8;
    }
}
