...
void func_80260BC4(struct MarioState *m, u32 actionState, s32 animFrame, u32 sound) {
    if (m->actionState == actionState && m->marioObj->header.gfx.unk38.animFrame == animFrame) {
        play_sound(sound, m->marioObj->soundOrigin);
    }
}
...
s32 act_start_sleeping(struct MarioState *m) {
    ...
    if (m->actionState == 2) {
        if (sp24 == -1) {
            play_sound(SOUND_MARIO_YAWNING, m->marioObj->soundOrigin);
        }
    }

    if (m->actionState == 1) {
        if (sp24 == -1) {
            play_sound(SOUND_MARIO_IMA_TIRED, m->marioObj->soundOrigin);
        }
    }
#else
    if (m->actionState == 2) {
    ...
    switch (m->actionState) {
        case 0: {
            ...
            if (sp24 == 0x31) {
                ...
                play_sound(SOUND_MARIO_PANTING_COLD, m->marioObj->soundOrigin);
            }
            if (sp24 == 7 || sp24 == 0x51) {
                play_sound(SOUND_ACTION_CLAP_HANDS_COLD, m->marioObj->soundOrigin);
            }
...
s32 act_coughing(struct MarioState *m) {
    ...
    if (sp1C == 0x19 || sp1C == 0x23) {
        play_sound(SOUND_MARIO_COUGHING3, m->marioObj->soundOrigin);
    }

    if (sp1C == 0x32 || sp1C == 0x3A) {
        play_sound(SOUND_MARIO_COUGHING2, m->marioObj->soundOrigin);
    }

    if (sp1C == 0x47 || sp1C == 0x50) {
        play_sound(SOUND_MARIO_COUGHING1, m->marioObj->soundOrigin);
    }
...
s32 act_first_person(struct MarioState *m) {
    ...
    if (m->actionState == 0) {
        func_80248C28(2);
        func_80285BD8(m->thisPlayerCamera, 6, 0x10);
        m->actionState = 1;
    } else {
