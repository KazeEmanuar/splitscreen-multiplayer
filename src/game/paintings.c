...
float gPaintingLuigiXPos, gPaintingLuigiYPos, gPaintingLuigiZPos;
...
float find_mario_y_position_on_painting(struct PaintingData *painting) {
    float mario_y_offset_from_painting = gPaintingMarioYPos - painting->vYPos;
    float luigi_y_offset_from_painting = gPaintingLuigiYPos - painting->vYPos;

    if (mario_y_offset_from_painting < 0.0) {
        mario_y_offset_from_painting = 0.0; // If Mario is past the left side, return the left side
    } else if (mario_y_offset_from_painting > painting->vSize) {
        mario_y_offset_from_painting =
            painting->vSize; // If Mario is past the right side, return the right side
    }

    if (luigi_y_offset_from_painting < 0.0) {
        luigi_y_offset_from_painting = 0.0; // If Mario is past the left side, return the left side
    } else if (luigi_y_offset_from_painting > painting->vSize) {
        luigi_y_offset_from_painting =
            painting->vSize; // If Mario is past the right side, return the right side
    }

    return mario_y_offset_from_painting * (mario_y_offset_from_painting < luigi_y_offset_from_painting)
           + luigi_y_offset_from_painting
                 * (mario_y_offset_from_painting >= luigi_y_offset_from_painting);
}

float find_mario_z_position_on_painting(struct PaintingData *painting) {
    float mario_z_offset_from_painting = gPaintingMarioZPos - painting->vZPos;
    float luigi_z_offset_from_painting = gPaintingLuigiZPos - painting->vZPos;

    if (mario_z_offset_from_painting < 0.0) {
        mario_z_offset_from_painting = 0.0; // If Mario is past the left side, return the left side
    } else if (mario_z_offset_from_painting > painting->vSize) {
        mario_z_offset_from_painting =
            painting->vSize; // If Mario is past the right side, return the right side
    }

    if (luigi_z_offset_from_painting < 0.0) {
        luigi_z_offset_from_painting = 0.0; // If Mario is past the left side, return the left side
    } else if (luigi_z_offset_from_painting > painting->vSize) {
        luigi_z_offset_from_painting =
            painting->vSize; // If Mario is past the right side, return the right side
    }

    return mario_z_offset_from_painting * (mario_z_offset_from_painting < luigi_z_offset_from_painting)
           + luigi_z_offset_from_painting
                 * (mario_z_offset_from_painting >= luigi_z_offset_from_painting);
}
...
float find_mario_x_position_on_painting(struct PaintingData *painting) {
    ...
    float luigi_x_offset_from_painting = gPaintingLuigiXPos - painting->vXPos;
    ...
    if (luigi_x_offset_from_painting < 0.0) {
        luigi_x_offset_from_painting = 0.0; // If Mario is past the left side, return the left side
    } else if (luigi_x_offset_from_painting > painting->vSize) {
        luigi_x_offset_from_painting =
            painting->vSize; // If Mario is past the right side, return the right side
    }

    return mario_x_offset_from_painting * (mario_x_offset_from_painting < luigi_x_offset_from_painting)
           + luigi_x_offset_from_painting
                 * (mario_x_offset_from_painting >= luigi_x_offset_from_painting);
}
...
void painting_update_floors(struct PaintingData *painting) {
    ...
    if (gPaintingMarioFloorType
        == paintingId * 3 + SURFACE_PAINTING_WOBBLE_A6) { // check if Mario's current floor is one of
                                                          // the special floors
    ...
    if ((gPaintingMarioYPos < painting->vYPos) || (gPaintingLuigiYPos < painting->vYPos)) {
...
Gfx *Geo18_802D5D0C(s32 run, UNUSED struct GraphNode *node, UNUSED f32 c[4][4]) {
    ...
    if (run != TRUE) {
        ...
        if (alternateFloor) {
            ...
            find_floor(gLuigiObject->oPosX, gLuigiObject->oPosY, gLuigiObject->oPosZ, &surface);
        }
        ...
        gPaintingLuigiXPos = gLuigiObject->oPosX;
        gPaintingLuigiYPos = gLuigiObject->oPosY;
        gPaintingLuigiZPos = gLuigiObject->oPosZ;
    }
    return NULL;
}
