...
//KAZEFIX this edit fixes most invisible walls using frames invis wall fix
//KAZEFIX fixed elevators cucking your jump when they move upwards
s32 perform_air_quarter_step(struct MarioState *m, Vec3f intendedPos, u32 stepArg) {
    ...
    if (nextPos[1] <= floorHeight) {
        if (gMarioObject->platform == NULL | (m->vel[1]<=0)){
