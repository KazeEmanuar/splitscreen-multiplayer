...
void point_light(s16 x, s16 y, s16 z, s16 size, f32 shade, s16 size1, f32 shade1) {
    ...
    if (gMarioStates[gCurrentArea->camera->cameraID].pos[0] >= (x - size1)
        && gMarioStates[gCurrentArea->camera->cameraID].pos[0] <= x + size1
        && gMarioStates[gCurrentArea->camera->cameraID].pos[1] >= (y - size1)
        && gMarioStates[gCurrentArea->camera->cameraID].pos[1] <= y + size1
        && gMarioStates[gCurrentArea->camera->cameraID].pos[2] >= (z - size1)
        && gMarioStates[gCurrentArea->camera->cameraID].pos[2] <= z + size1) {
    ...
    if (gMarioStates[gCurrentArea->camera->cameraID].pos[0] >= (x - size)
        && gMarioStates[gCurrentArea->camera->cameraID].pos[0] <= x + size
        && gMarioStates[gCurrentArea->camera->cameraID].pos[1] >= (y - size)
        && gMarioStates[gCurrentArea->camera->cameraID].pos[1] <= y + size
        && gMarioStates[gCurrentArea->camera->cameraID].pos[2] >= (z - size)
        && gMarioStates[gCurrentArea->camera->cameraID].pos[2] <= z + size) {
